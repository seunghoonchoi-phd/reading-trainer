...
const MYTHS = [
  { t: '눈을 훈련하면 시야폭이 넓어져 한 줄/한 단락을 한눈에 읽는다', d: '지각폭은 주의(attention) 기반이라 고정돼 있습니다. 영어는 고정점 오른쪽 ~14–15자가 보여도 *단어로 식별*되는 건 ~7–8자뿐, 중국어는 오른쪽 ~3자. 주변 글자를 확대해도 폭은 안 커졌습니다(주변시 확대 실험 실패). 한눈에 읽는 느낌은 폭이 아니라 어휘 인지가 빨라진 *결과*입니다.' },
  ...
  { t: '속발음(머릿속 소리)을 없애야 빨라진다', d: '속발음은 구조적이라 가장 빠른 독자에게도 EMG/fMRI로 관찰됩니다. 억제하면 특히 어려운 글에서 이해가 떨어집니다. 가능한 건 난이도에 맞춘 음운부하 *페이싱*뿐, "내면의 목소리 죽이기"가 아닙니다.' },
...
const OVERLOAD = [
  ['합법적 부하 (the legal lift)', '이해 충실도만 빼고 모든 축을 과부하해도 됩니다. 속도는 *관찰하는 출력*이지 *강제하는 입력*이 아닙니다. 이해 미니체크가 기준 아래로 떨어지면 자동으로 deload(완화) — 밀어붙이지 않습니다. 힘이 엔진이 아니라, 이해하며 성공적으로 반복하는 것이 엔진입니다.'],
  ...
  ['회복 · 간격 · 수면', '간격 학습은 벼락치기를 이기는 가장 재현된 발견 중 하나이고, 효과는 *나중* 시험에서 드러납니다(당일이 아니라). 수면은 새로 익힌 *단어·의미*를 공고화합니다. 다만 수면이 읽기 *속도*를 빠르게 한다는 근거는 약합니다 — 속도는 절차적 기술입니다. 그래서 일정만 잡고 과장하지 않습니다.'],
...
export function renderTheory(root) {
  ...
  const drillCards = DRILLS.map(d => h('div', { class: 'note', style: { marginBottom: '10px' } },
    h('div', { class: 'row', style: { gap: '8px', marginBottom: '4px' } }, h('span', null, d.icon), h('b', null, d.name)),
