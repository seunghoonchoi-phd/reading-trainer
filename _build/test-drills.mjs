...
check('내장 지문이 없는 환경에서는 새 글 선택기가 지문을 만들지 않는다', () => {
  store.resetEverything();
  store.setDifficulty('en', 2, 'manual');
  const first = shared.pickUnseenPassage('en', { tier: 1 });
  assert.equal(first, null);
});

check('내장 지문이 없는 환경에서는 평가용 새 글도 반환하지 않는다', () => {
  store.resetEverything();
  store.setDifficulty('en', 1, 'manual');
  assert.equal(shared.pickAssessmentPassage('en', { tier: 1 }), null);
});

check('내장 지문이 없는 환경에서는 반복읽기 전이 후보도 만들지 않는다', () => {
  store.resetEverything();
