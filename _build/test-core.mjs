...
equal(chinese[0].fixed_expressions.length, 16, 'registered Chinese passage keeps source-grounded expressions and sentence patterns');
equal(chinese[0].fixed_expressions.find(item => item.kind === 'connector')?.term, '虽然……但/但是……', 'registered Chinese passage keeps the connector pattern');
check(chinese[0].fixed_expressions.every(item => item.source_excerpt && item.kind), 'every registered Chinese learning item has a source excerpt and category');
